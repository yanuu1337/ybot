...
export default class extends Command {
    ...
    description = 'Slap someone\'s cheek!';
    isSlashCommand = true;
    data = new SlashCommandBuilder().addUserOption(usr => usr.setName("member").setRequired(true).setDescription("The user you want to slap"));
    async execute(client: ArosClient, message: Message<boolean>, args: string[], guild: GuildInterface | null) {
        ...
        if(message.reference) {
            ...
            const embed = EmbedFactory.generateInfoEmbed(`Slap`, `${message.member!.displayName} ${Utility.translate(guild?.language, 'fun/actions:SLAPPED')} ${msg.member?.displayName}!`)
        ...
        const embed = EmbedFactory.generateInfoEmbed(`Slap`, `${message.member!.displayName} ${Utility.translate(guild?.language, 'fun/actions:SLAPPED')} ${mem.displayName}!`)
        embed.setImage(json.url)
        return message.reply({embeds: [embed]})
    }
    ...
    async executeSlash(client: ArosClient, interaction: CommandInteraction<CacheType>, guild: GuildInterface | null, isInDms?: boolean): Promise<any> {
        ...
        const embed = EmbedFactory.generateInfoEmbed(`Slap`, `${author?.displayName} ${Utility.translate(guild?.language, 'fun/actions:SLAPPED')} ${member?.displayName}!`).setImage(json.url);
