...
export default class extends Command {
    ...
    isSlashCommand = true;
    data = new SlashCommandBuilder().addUserOption(usr => usr.setName("member").setRequired(true).setDescription("The user you want to hug"));
    async execute(client: ArosClient, message: Message<boolean>, args: string[], guild: GuildInterface | null) {
        ...
        if(message.reference) {
            ...
            const embed = EmbedFactory.generateInfoEmbed(`Hug`, `${message.member!.displayName} ${Utility.translate(guild?.language, 'fun/actions:HUGGED')} ${msg.member?.displayName}!`)
        ...
        const embed = EmbedFactory.generateInfoEmbed(`Hug`, `${message.member!.displayName} ${Utility.translate(guild?.language, 'fun/actions:HUGGED')} ${mem.displayName}!`)
        embed.setImage(json.url)
        return message.reply({embeds: [embed]})
    }
    ...
    async executeSlash(client: ArosClient, interaction: CommandInteraction<CacheType>, guild: GuildInterface | null, isInDms?: boolean): Promise<any> {
        ...
        const embed = EmbedFactory.generateInfoEmbed(`Hug`, `${author?.displayName} ${Utility.translate(guild?.language, 'fun/actions:HUGGED')} ${member?.displayName}!`).setImage(json.url);
